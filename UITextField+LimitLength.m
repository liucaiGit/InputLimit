...
static const void *bwLimitNumkey = &bwLimitNumkey;

static const void *bwLimitCharKey = &bwLimitCharKey;

@implementation UITextField (LimitLength)
- (void)setMaxCharLength:(NSNumber *)maxCharLength {
    objc_setAssociatedObject(self, bwLimitCharKey, maxCharLength, OBJC_ASSOCIATION_COPY_NONATOMIC);
    
    [self addTextCharDidChagedNotification];
}

- (NSNumber *)maxCharLength {
    return objc_getAssociatedObject(self, bwLimitCharKey);
}

- (void)setMaxLength:(NSNumber *)maxLength {
    objc_setAssociatedObject(self, bwLimitNumkey, maxLength, OBJC_ASSOCIATION_COPY_NONATOMIC);
    
    [self addTexDidChangedNotification];
}

- (NSNumber *)maxLength {
    return objc_getAssociatedObject(self, bwLimitNumkey);
}
...
- (void)addTextCharDidChagedNotification {
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(textFieldTextCharDidChanged) name:UITextFieldTextDidChangeNotification object:nil];
}
